...
function DonationSection() {
    ...
    const { error } = await stripe.redirectToCheckout({
        lineItems: [{
            price: donationAmountPicked, // Replace with the ID of your price
            quantity: 1,
        }],
        mode: 'payment',
  ...
  return (
    <div className="container">
        <button className="btn btn-primary" role="link" onClick={handleClick} value="5">
            Give $5.00
        </button> 
        <button className="btn btn-primary" role="link" onClick={handleClick} value="25">
            Give $25.00
        </button>   
    </div>
  );
