...
function Sermons() {
	...
	useEffect(() => {
		async function getSermonPageData() {
			const sermonPageDataResponse = await store.getSermonData(
				'https://hillcitysc.com//wp-json/wp/v2/wpfc_sermon?per_page=100'
			);
	...
	if (store && store.sermonData && store.sermonData.length > 0) {
		...
		currentSermons = store.sermonData.slice(
			indexOfFirstSermon,
			indexOfLastSermon
		);
	}
					...
					<div>
						{store.sermonData.length > 0 ? (
							<>
								<PageHeader
									headerData={null}
									sermonHeaderData={store.sermonData[0]}
								/>
							</>
						) : null}
					...
					{store.sermonData.length > 0 ? (
						<>
							...
							<div className='container'>
								<Pagination
									sermonsPerPage={postsPerPage}
									totalSermons={store.sermonData.length}
									paginate={paginate}
								/>
							</div>
						</>
