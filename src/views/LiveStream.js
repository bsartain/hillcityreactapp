...
function LiveStream() {
	...
	useEffect(() => {
		async function getOrderOfService() {
			const orderOfServiceDataResponse = await store.getSermonData(
							...
							return (
								<div key={index}>
									...
									<div className='page-content-title container'>
										...
										<h3>
											WELCOME! OUR SERVICE WILL BEGIN AT 10:00 AM ON{' '}
											{sundayDate}
										</h3>
									</div>
									<div
										className='container live-stream-content'
										...
										<ReactToPrint
											trigger={() => (
												<button className='btn btn-primary'>
													<i className='now-ui-icons files_paper'></i> Print
													Order Of Service
												</button>
											)}
											content={() => ref.current}
											...
											pageStyle={pageStyle}
										/>
										{store.orderOfServiceData.length === 0 ? null : (
