...
function MediaDrawer() {
  ...
  return useObserver(() =>
    store.sermonStore.singleSermonData.length === 0 ? null : (
      <div>
          ...
          <div className="media-player-container">
            <div className="media-player-inner-container">
              <div className="media-player-details">
                {setBackgroundImage(store.sermonStore.singleSermonData)}
                <div className="media-player-sermon-info">
                  <div dangerouslySetInnerHTML={{ __html: store.sermonStore.singleSermonData.title }} />
                  <hr />
                  <div style={{ display: 'flex' }}>
                    <div style={{ borderRight: '1px solid #515151', marginRight: '5px', paddingRight: '5px' }}>{store.sermonStore.singleSermonData.preacher.map((item) => item.name)}</div>
                    <div>{formatDate(store.sermonStore.singleSermonData.date)}</div>
                  </div>
                </div>
              </div>
              <AudioPlayer
